 ...
 * reconciliation-claim coordination proofs for archiveLegacyInvoice().
 * Mirrors test/integration/invoices/issue-reconciliation-coordination.test.ts
 * exactly, substituting Legacy Archive's own service/error codes.
 */
...
describe("archiveLegacyInvoice — reconciliation coordination", () => {
  ...
  it("finalization wins: after a normal successful archival, the row is REFERENCED and reconciliation never claims it", async () => {
  ...
  it("reconciliation claim wins: injected mid-flight, the whole Invoice+ledger transaction rolls back — original status/fields preserved", async () => {
    const invoice = await seedLegacyInvoice(fixtures);
    ...
    const after = await prisma.invoice.findUniqueOrThrow({ where: { id: invoice.id } });
    expect(after.status).toBe("SENT"); // the source Invoice's own original status is untouched
  ...
  it("compensation cannot overwrite a row the reconciliation worker currently holds claimed", async () => {
    ...
    try {
      ...
      expect(after.cleanupClaimToken).toBe(claimToken);
    } finally {
      await prisma.invoicePdfArchiveObject.delete({ where: { id: archiveId } });
    }
  });

  it("pre-upload ownership loss: when the ownership recheck finds no matching row, deps.upload is never called and ARCHIVE_FAILED is returned", async () => {
    const invoice = await seedLegacyInvoice(fixtures);
  ...
  it("pre-upload ownership query failure: when the ownership recheck rejects, deps.upload is never called, the bounded ARCHIVE_FAILED result is returned, and the PENDING_UPLOAD ledger row survives untouched", async () => {
    const invoice = await seedLegacyInvoice(fixtures);
    const uploadSpy = vi.fn(async () => ({ ok: true as const }));
    let capturedArchiveId = "";
